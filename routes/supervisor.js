...
//       if (password !== user.password) return res.status(401).json({ status: false, message: "Invalid password" });
...
//         if (existing) return res.status(400).json({ status: false, message: "Username already exists" });
...
//         if (!supervisor) return res.status(404).json({ status: false, message: "Supervisor not found" });
...
//   // ---------------- GET ALL SUPERVISORS (hide admins)
...
//       if (!id || !type) return res.status(400).json({ status: false, message: "ID and type required" });
